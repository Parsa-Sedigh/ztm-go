...
var args struct {
	/* When we run this application, we will be able to specify the DbPath manually on the command line or we will be able to set the MAILINGLIST_DB environment variable.*/
...
func main() {
	...
	if args.BindJson == "" {
		// if you don't set an IP address, it will be default to localhost.
		args.BindJson = ":8080"
	}
	if args.BindGrpc == "" {
		args.BindGrpc = ":8081"
