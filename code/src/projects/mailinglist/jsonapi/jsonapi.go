...
/* When we receive an http req from clients, the body of the req which contains all the data, it's going to be in io.Reader type  */
func fromJson[T any](body io.Reader, target T) {
	buf := new(bytes.Buffer)
	buf.ReadFrom(body)

	/* When we use the Unmarshal() function, we're gonna be converting bytes into a target structure. The bytes are the bytes that were read from the body of
	the req and those are saved into a buffer and the target is a pointer to the target data structure taht will be supplied by the caller and it can be any type(T). */
	json.Unmarshal(buf.Bytes(), &target)
}

// with http.ResponseWriter, we're gonna be able to write any data into http response.
func returnJson[T any](w http.ResponseWriter, withData func() (T, error)) {
	...
	if serverErr != nil {
		w.WriteHeader(500)
		/* The Marshal() function is gonna take some structure in go and it's gonna get converted into json data with Marshal() . */
...
func returnErr(w http.ResponseWriter, err error, code int) {
	// interface{} means we can use any type we want.
...
func CreateEmail(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		/* At this point, the new email is saved to the DB and now we just do a GetEmail to return all the info out of DB in a JSON object. */
		returnJson(w, func() (interface{}, error) {
...
/* this function serves up each handler.

bind will be the bind address, so it'll be the IP address that the server is running on.*/
func Serve(db *sql.DB, bind string) {
	...
	// ListenAndServe() is gonna setup an http server
