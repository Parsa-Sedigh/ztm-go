...
/* We make out own structure for a custom error type, that way we can return the invalid input and it can be inspected later. */
type TimeParseError struct {
	msg   string
	input string
}

// implement the error interface for our TimeParseError struct:
...
func ParseTime(input string) (Time, error) {
	...
	if len(components) != 3 {
		...
		if err != nil {
			return Time{}, &TimeParseError{fmt.Sprintf("Error parsing miunte: %v", err), input}
		}
